...
export default function SingleBallotPageClient() {
  ...
  const loadBallotData = useCallback(async () => {
    ...
    try {
      ...
      if (connection) {
        ...
        // Fetch whitelist and hasVoted if user address is available
        if (address) {
  ...
  }, [contractAddress, getBallot, provider, signer, address]);
  ...
  const handleVote = async (choiceName: string) => {
    ...
    if (isWhitelisted === false) {
      return alert("Your address is not whitelisted for this election.");
    }
    if (userHasVoted) {
    ...
    try {
      ...
      // Reload on-chain details after successful vote
  ...
  // Color options for charts/progress bars
  const colors = [
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-8 space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-3 space-y-6">
          <Card className="border-border/60 bg-card/30 backdrop-blur-sm shadow-sm h-full flex flex-col">
            <CardHeader>
              ...
              <CardDescription>
                Sign with your wallet to vote. The Gelato network will execute the transaction on-chain for free.
              </CardDescription>
            </CardHeader>
            <CardContent className="flex-1 flex flex-col justify-between space-y-6">
              ...
              {!address ? (
                ...
                <div className="p-4 rounded-xl border border-border bg-muted/20 text-muted-foreground text-sm flex items-start gap-3">
                  ...
                  <div>
                    ...
                    Checking if your address ({address.slice(0, 6)}...{address.slice(-4)}) is in the whitelist.
                  </div>
                </div>
              ) : isWhitelisted === false ? (
                <div className="p-4 rounded-xl border border-rose-500/20 bg-rose-500/5 text-rose-600 dark:text-rose-400 text-sm flex items-start gap-3">
                  ...
                  <div>
                    ...
                    Your connected wallet ({address.slice(0, 6)}...{address.slice(-4)}) is not whitelisted to vote in this ballot.
                  </div>
                </div>
              ) : userHasVoted ? (
                <div className="p-4 rounded-xl border border-emerald-500/20 bg-emerald-500/5 text-emerald-600 dark:text-emerald-400 text-sm flex items-start gap-3">
                  ...
                  <div>
                    <span className="font-semibold block mb-0.5">Vote Recorded</span>
                    Your address ({address.slice(0, 6)}...{address.slice(-4)}) has successfully voted on this contract.
                  </div>
                </div>
              ) : isExpired ? (
                <div className="p-4 rounded-xl border border-rose-500/20 bg-rose-500/5 text-rose-600 dark:text-rose-400 text-sm flex items-start gap-3">
                ...
                <div className="p-4 rounded-xl border border-emerald-500/20 bg-emerald-500/5 text-emerald-600 dark:text-emerald-400 text-sm flex items-start gap-3">
                  ...
                  <div>
                    ...
                    Your wallet is whitelisted! Choose one of the options below to cast your sponsored vote.
                  </div>
                </div>
              )}
              ...
              {deadline && (
                <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-muted/20 border border-border/50 text-xs">
                  ...
                  <div className="space-y-1">
                    <span className="text-muted-foreground block font-medium uppercase tracking-wider">Booths</span>
                    <span className="font-semibold text-foreground flex items-center gap-1">
        ...
        {/* Live Results Display (Right 2 Columns) */}
            ...
            <CardContent className="flex-1 flex flex-col justify-between space-y-6">
              ...
              {totalVotes === 0 && (
                <div className="py-12 flex flex-col items-center justify-center text-center text-muted-foreground space-y-2 border border-dashed rounded-2xl bg-muted/15">
                  ...
                  <p className="text-[10px] max-w-[200px] leading-relaxed">Be the first to cast a cryptographic signature vote!</p>
                </div>
              )}

              <div className="pt-4 border-t border-border/40 text-[10px] text-muted-foreground/80 leading-relaxed">
                Voting on Sepolia uses cryptographic signatures to prove authorship. The Gelato relay sponsor pays the gas fee, and the contract handles on-chain storage.
              </div>
