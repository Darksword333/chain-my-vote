...
export async function computeIdentityCommitment(idNumber: string): Promise<bigint> {
    const idBigInt = BigInt(idNumber);
    return poseidon1([idBigInt]);
}

export async function createMerkleTree(idNumbers: string[]): Promise<MerkleTree> {
    const tree = new MerkleTree(16); // 16 levels as per semaphore_vote.circom
    
    for (const id of idNumbers) {
        const commitment = await computeIdentityCommitment(id);
